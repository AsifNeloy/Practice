...
void updateKeyboard(int value) //upK
...
{
    if(xr >280.0) //ending point from left to right
        xr = -30.0f; //in loop starts from left starting point and goes to right

    if(xr <-30.0) //ending point from right to left
        xr = 280.0f;  //in loop starts from right starting point and goes to left
...
//Sarafat's Part
void handleScene(unsigned char key, int x, int y) {
...
int main(int argc, char** argv) {
	...
	glutKeyboardFunc(handleKeypress);  //Move Car
	glutKeyboardFunc(handleScene);     //Change the Scene
    glutTimerFunc(20,updateJet1Left , 0);
    glutTimerFunc(20,updateJet1Right , 0);
    glutTimerFunc(10,updateJet2Keyboard,0);
